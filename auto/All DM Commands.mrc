    ...
    if ($nick == %p1 [ $+ [ $chan ] ] && %turn [ $+ [ $chan ] ] == 1) || ($nick == %p2 [ $+ [ $chan ] ] && %turn [ $+ [ $chan ] ] == 2) {
      ...
      if ($ini(equipment.ini,$right($1,-1))) {
        if (!$readini(equipment.ini,$right($1,-1),$nick)) {
          else notice $nick You have to unlock this weapon before you can use it.
          halt
        }
      }
...
alias damage {
  ...
  if (%sitem.belong [ $+ [ $4 ] ] == on) && ($($+(%,hp,$player($2,$4),$4),2) >= 1) {
    ...
    dec $+(%,hp,$player($2,$4),$4) %extra [ $+ [ $4 ] ]
    msg $4 $logo(DM) $s1($1) whips out their Belong Blade and deals $s2(%extra [ $+ [ $4 ] ]) extra damage. HP $+($chr(91),$s2($iif($($+(%,hp,$player($2,$4),$4),2) < 1,0,$v1)),$chr(93)) $hpbar($($+(%,hp,$player($2,$4),$4)))
  }
  if ($readini(sitems.ini,kh,$2)) && ($r(1,30) == 1) {
    inc $+(%,hp,$player($2,$4),$4) $calc($replace(%hit [ $+ [ $4 ] ],$chr(32),$chr(43)))
    msg $4 $logo(DM) KHobbits lends $s1($2) his KHonfound Ring, allowing $s1($2) to avoid the attack. HP $+($chr(91),$s2($iif($($+(%,hp,$player($2,$4),$4),2) < 1,0,$v1)),$chr(93)) $hpbar($($+(%,hp,$player($2,$4),$4)))
  ...
  if ($readini(sitems.ini,allegra,$2)) && ($r(1,30) == 1) && ($($+(%,hp,$player($2,$4),$4),2) >= 1) {
    ...
    msg $4 $logo(DM) Allegra gives $s1($2) Allergy pills, healing $s2(%allegra.heal [ $+ [ $4 ] ]) HP. HP $+($chr(91),$s2($iif($($+(%,hp,$player($2,$4),$4),2) < 1,0,$v1)),$chr(93)) $hpbar($($+(%,hp,$player($2,$4),$4)))
  }
  if ($($+(%,hp,$player($2,$4),$4),2) < 1) { 
    if ($readini(sitems.ini,beau,$2)) && ($r(1,15) == 1) { 
      set $+(%,hp,$player($2,$4),$4) 1
      msg $4 $logo(DM) $s1($2) $+ 's Beaumerang brings $s1($2) back to life, barely. HP $+($chr(91),$s2($iif($($+(%,hp,$player($2,$4),$4),2) < 1,0,$v1)),$chr(93)) $hpbar($($+(%,hp,$player($2,$4),$4)))
      set %turn [ $+ [ $4 ] ] $iif($player($1,$4) == 1,2,1)
      halt
    }
    dead $4 $2 $1
