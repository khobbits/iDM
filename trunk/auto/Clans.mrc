on $*:TEXT:/^[!@.]delmem .*/Si:*: {
  ...
  if ($db.get(user,login,$nick) === 0) { notice $nick You have to login before you can use this command. ( $+ $s2(/msg $me $iif($db.get(user,pass,$nick),id,reg) pass) $+ ) (Don't use your RuneScape password) | halt }
