...
alias toplist {
  ...
  var %output, %i = 0
  var %sql = SELECT * FROM $db.tquote($1)) WHERE c1 = $db.quote($1)) AND c2 NOT LIKE '~banned~%' ORDER BY c3 +0 DESC LIMIT 0, $+ $2
...
alias ranks {
  ...
  if ($2 isnum) {
    ...
    var %sql = SELECT * FROM $db.tquote($1) WHERE c1 = $db.quote($1) AND c2 = $db.quote($2) LIMIT 0,1
