...
on *:TEXT:!lend*:#: {
  ...
  if ($3 == $nick) { halt }
  if (!$2) || (!$readini(lent.ini,lendable,$2)) { notice $nick Please specify the correct item of equipment to lend. For example: !lend Ags Allegra | halt }
  if (!$3) || (!$3 ison $chan) { notice $nick Please specify a nick to lend the item to whome is on the channel you are currently in. | halt }
  if (!$readini(login.ini,login,$nick)) { notice $nick You must be logged in to have the capablity of lending items. | halt }
  if (!$readini(equipment.ini,$2,$nick)) { notice $nick You currently don't have $qt($2) in your equipment. | halt }
  if ($readini(equipment.ini,$2,$3)) { notice $nick $qt($3) already has this piece of equipment. | halt }
...
alias itemlend {
  ...
  notice $1 $logo(Lent) $2 has been successfully been retrieved from $3
