on $*:TEXT:/^[!@.](dm|gwd|boss|pvm)\b/Si:#: {
  if ($chan == $supportchan) && ($nick !isop $chan) { halt }
  if ($chan == #stats) { halt }
  if ($chan == #idm || # == $staffchan) && ($me != iDM) { halt }
  if ((%dm.spam [ $+ [ $nick ] ]) || (%wait. [ $+ [ $chan ] ])) { halt }
  if ($isbanned($nick)) { putlog $logo(Banned) $nick tried to dm on $chan | halt }
  if ($update) { notice $nick $logo(ERROR) iDM is currently disabled, please try again shortly | halt }
  if (*m* iswmcs $chan($chan).mode && $nick($chan,$me,r)) { notice $nick $logo(ERROR) You currently have +m enabled and I do not have correct access to talk, please voice me | halt }
  if (stake isin $1) { notice $nick $logo(ERROR) Staking is currently disabled. Please try again later | halt }
  if ($nick == idmgod) { halt }
...
alias startdm {
  ...
  if ($hget(%chan)) {
    ...
    elseif ((gwd isin $1) || (boss isin $1) || (pvm isin $1)) {
      ...
      if (%chan == #idm.newbies) {
        msgsafe %chan $logo(DM) $s1(%nick) $winloss(%nick) has requested a DM! You have $s2(25 seconds) to accept.
        .timer $+ %chan 1 25 autoidm.run %chan
      }
      else {
...
alias winloss {
  if ($2) {
    ...
    var %p1ratio $round($calc(%p1win / %p1loss),2)
    var %p2ratio $round($calc(%p2win / %p2loss),2)
    var %p1 $s2($chr(91)) $+ Wins $s1($iif(%p1win,$bytes($v1,bd),0)) Losses $s1($iif(%p1loss,$bytes($v1,bd),0)) ( $+ %p1ratio $+ ) $+ $s2($chr(93)) $iif($hget($1,sitems),,(NA))
    var %p2 $s2($chr(91)) $+ Wins $s1($iif(%p2win,$bytes($v1,bd),0)) Losses $s1($iif(%p2loss,$bytes($v1,bd),0)) ( $+ %p2ratio $+ ) $+ $s2($chr(93)) $iif($hget($2,sitems),,(NA))
...
on $*:TEXT:/^[!@.]enddm/Si:#: {
  ...
  elseif (($nick == $hget($chan,p2))) {
    ...
    if ($calc($ctime - $hget($chan,dm.time)) < 30) {
