...
alias damage {
  ...
  if ($hget($2,poison) < 6 && $gettok($poisoner($3),1,32)) && (($r(1,$v1) == 1) || (($hget($1,snake)) && (!$hget($2,poison)) && ($gettok($poisoner($3),2,32) < 7))) {
  ...
  var %admitem $r(1,50)
  if ($hget($1,belong)) && (%admitem == 50) && (%hp2 >= 1) {
  ...
  elseif ($hget($2,allegra)) && (%admitem == 46) && (%hp2 >= 1) && (%hp2 < 99) {
  ...
  if ((!$hget($4,gwd.time)) && (<idm>* !iswm $1) && ($hget($2,jade)) && (%admitem == 44) && (%temp.hit < 70) && (%hp1 >= 1) && (%hp2 >= 1) && ($calc($replace(%hit,$chr(32),$chr(43))) != 0)) {
    var %jhit $floor($calc(%temp.hit * .5)) 
    var %fhit $iif(%jhit > %hp1, %hp1,%jhit)
    dec %hp1 %jhit
    msgsafe $4 $logo(%logo) $s1($2) goes tit for tat with $s1($autoidm.nick($1)) and deals $s2(%fhit) damage back. $hpbar(%hp1,%mhp1)
  }
  elseif ($hget($2,kh)) && (%admitem == 42) && (%temp.hit < 70) && (%hp2 >= 1) && (%hp1 < 99) && ($calc($replace(%hit,$chr(32),$chr(43))) != 0) {
    inc %hp2 %temp.hit
    msgsafe $4 $logo(%logo) KHobbits uses his KHonfound Ring to let $s1($2) avoid the damage. $hpbar(%hp2,%mhp2)
  }
  elseif ($hget($2,support)) && (%admitem == 40)  && (%temp.hit < 70) && (%hp2 >= 1) && ($calc($replace(%hit,$chr(32),$chr(43))) != 0) {
  ...
  if (%dmg-dealt >= 99) { db.user.set achievements 1hit $autoidm.acc($1) 1 }
