...
alias money {
  ...
  return $s1(Money) $+ : $iif(%money,$s2($bytes($v1,bd)) $+ gp ( $+ %rank $+ ),$s2(0) $+ gp) $s1(Wins) $+ : $iif(%wins,$s2($bytes($v1,bd)),$s2(0)) $s1(Losses) $+ : $iif(%losses,$s2($bytes($v1,bd)),$s2(0)) %ratio
}
...
alias equipment {
  ...
  if ($hget(>equip_item,arctic)) { var %e %e Arctic:Arrows $+ $iif($v1 > 1,$+($chr(40),$v1,$chr(41))) }
  if ($hget(>equip_item,admin)) { var %e %e Magical:Ring }
  if ($hget(>equip_item,ticket)) { var %e %e Ticket }
