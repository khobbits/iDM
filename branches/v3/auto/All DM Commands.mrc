...
alias damage {
  ...
  var %msg $logo(%logo) $s1($1) $replace($action($3),$eval(%p1%,0),$s1($replace($2,$chr(58),$chr(32))),$eval(%attack%,0),$attackname($3))
  ...
  var %heal 0
  var %healer $r(1,$v1)
  if ($gettok($healer($3),1,32)) && (%healer == 1) && (%hitdmg != 0) && (%hp1 < 99) {
  ...
  if (%healer == 1) { var %msg %msg $+ . $s1($replace($2,$chr(58),$chr(32))) $hpbar(%hp2) - $s1($replace($1,$chr(58),$chr(32))) $hpbar(%hp1) }
  ...
  if ($hget($1,belong)) && ($r(1,100) >= 99) && (%hp2 >= 1) {
    ...
    inc %dmg-dealt %extra
    dec %hp2 %extra
    msgsafe $4 $logo(%logo) $s1($1) whips out their Bêlong Blade and deals $s2(%extra) extra damage. $hpbar(%hp2)
  }
  if ($hget($2,allegra)) && ($r(1,100) >= 99) && (%hp2 >= 1) && (%hp2 < 99) {
    var %extraup $iif(%hp2 >= 84,$calc(99- %hp2),15)
    inc %hp2 %extraup
    msgsafe $4 $logo(%logo) Allêgra gives $s1($2) Allergy pills, healing $s2(%extraup) HP. $hpbar(%hp2)
  }
  elseif ($hget($2,kh)) && ($r(1,100) >= 99) && (%hp2 >= 1) {
    inc %hp2 $calc($replace(%hit,$chr(32),$chr(43)))
    msgsafe $4 $logo(%logo) KHobbits uses his KHonfound Ring to let $s1($2) avoid the damage. $hpbar(%hp2)
  }
  elseif ($hget($2,support)) && ($r(1,100) >= 99) && (%hp2 >= 1) {
    var %temp.hit $calc($replace(%hit,$chr(32),$chr(43)))
    inc %hp2 $floor($calc(%temp.hit / 2))
    msgsafe $4 $logo(%logo) $s1($2) uses THE SUPPORTER to help defend against $s1($1) $+ 's attacks. $hpbar(%hp2)
  }
  if (%hp2 < 1) {
    if (($hget($2,beau)) && ($r(1,50) >= 49) ) {
      var %hp2 1
      msgsafe $4 $logo(%logo) $s1($2) $+ 's Bêaumerang brings them back to life, barely. $hpbar(%hp2)
    }
  }
  hadd $1 hp %hp1
  hadd $2 hp %hp2
